 ...
 */
void D3_view(visualization::PCLVisualizer &viewer, std::vector<PointCloud<PointXYZ>::Ptr> &cloud_vec){
...
{ 
 ...
 for(int i = 0; i<planars_cloud_vec.size();i++){
    ...
    if(cloud_boundary->points.size()!= 645&&cloud_boundary->points.size()!= 334 &&cloud_boundary->points.size()>300 && cloud_boundary->points.size() < 0.5 * cloud->points.size()){
 ...
 */
int region_grow (pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud,std::vector<PointCloud<PointXYZ>::Ptr> &res_vec , float general_planar_height)
{
...
//  float general_planar_height =  get_planar_height(cloud);
  ...
  for(pcl::PointIndices inliers : clusters){
       ...
       if(get_planar_height(output) > general_planar_height && output->points.size() > 200){
            res_vec.push_back(output);
