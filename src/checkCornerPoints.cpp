...
int canny_find_boundary(Mat &img , Mat &canny_out ){
	...
	Canny(gray, canny_out, 20, 150, 3, true);
...
int hough_find_lins(vector<Vec4f> &lines,Mat &img, Mat &img_out){
	...
	HoughLinesP(img_out,lines,1,CV_PI/180,60,0,10);
    //HoughLinesP(img_out,lines,1,CV_PI/180,10,0,10);
	Scalar color = Scalar(0,0,255);
	for (size_t i = 0; i < lines.size(); i++)
	{
		Vec4f plines=lines[i];  //一个plines里边是四个点一条直线                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               
	    line(img,Point(plines[0],plines[1]),Point(plines[2],plines[3]),color,1,LINE_AA);
        //cout<<lines[i]<<endl;
	}
	mat_show("hough_img",img,500);
